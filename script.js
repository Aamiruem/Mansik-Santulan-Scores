...
//   const API_BASE = "https://mansik-santulan-scores.onrender.com";
...
(() => {
  ...
  const GAUGE_ARC_LENGTH = 314; // approx pi * r(100)
  const STORAGE_KEYS = {
    form: "mentalHealthFormDataV1",
    prediction: "mentalHealthPredictionV1",
  };

  // ---------------------------------------------------------
  // Keep form data and the latest prediction after page refresh
  // ---------------------------------------------------------
  function saveFormState() {
    ...
    localStorage.setItem(STORAGE_KEYS.form, JSON.stringify(savedValues));
  }
  ...
  function restoreFormState() {
    ...
    try {
      ...
      if (savedValues.stress_level) {
        stressHiddenInput.value = savedValues.stress_level;
        segGroup.querySelectorAll(".seg-btn").forEach((button) => {
          button.classList.toggle(
            "active",
            button.dataset.value === savedValues.stress_level
          );
        });
      }
    } catch (error) {
  ...
  function savePrediction(score) {
    localStorage.setItem(STORAGE_KEYS.prediction, String(score));
  }

  function restorePrediction() {
    const storedScore = localStorage.getItem(STORAGE_KEYS.prediction);
    if (storedScore === null) return false;

    const savedScore = Number(storedScore);
    if (Number.isFinite(savedScore)) {
      renderResult(savedScore);
      return true;
    }
    return false;
  }
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      saveFormState();
      savePrediction(data.predicted_mental_health_score);
      renderResult(data.predicted_mental_health_score);
    } catch (err) {
      renderError(
        "Can't reach the server",
  ...
  resetBtn.addEventListener("click", () => {
    ...
    segGroup.querySelectorAll(".seg-btn").forEach((button) => {
      button.classList.toggle(
        "active",
        button.dataset.value === stressHiddenInput.value
      );
    });
    showState("idle");
  });
  ...
  // Run once when the page opens or the browser refreshes.
  restoreFormState();
...
// Paste your complete FORM_STORAGE_KEY code here
const FORM_STORAGE_KEY = "mentalHealthFormData";
...
// Existing stress button code continues here
segGroup.querySelectorAll(".seg-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    // Save the selected stress button
    saveFormData();
  });
});
